...
class AppStructure:
    ...
    def create_launcher_script(self, archs: List[str]) -> None:
        ...
        template_path = Path(__file__).parent.parent / "templates" / "launcher.sh"
        
        if not template_path.exists():
            self.logger.error(f"Launcher template not found at {template_path}")
            raise FileNotFoundError(f"Launcher script template not found at {template_path}")
        
        # Copy the template file to the destination
        shutil.copy(template_path, launcher_path)
        
        # Make it executable
        os.chmod(launcher_path, 0o755)
        
        self.logger.info(f"Launcher script created at {launcher_path}")